"""End-to-end benchmark entry point for the PathRAG baseline."""
...
DEFAULT_PROJECT_DIR = REPOSITORY_ROOT / "benchmark" / "data" / "path_rag"
DEFAULT_DATASET = (
    REPOSITORY_ROOT / "benchmark" / "qa" / "dataset" / "sample_questions.json"
)
DEFAULT_RESULTS_DIR = REPOSITORY_ROOT / "benchmark" / "results" / "path_rag"

# PathRAG exposes a single hybrid retrieval pipeline; every question archetype
# therefore maps onto the same engine mode.
ADAPTIVE_SEARCH_METHODS = {
    "basic": "hybrid",
    "multi-vector": "hybrid",
    "graph-enhanced": "hybrid",
}
...
def prepare_corpus(
    *,
    ...
    """Extract PDFs into the unified corpus directory and initialise the project.

    The unified layout (``benchmark/data/corpus/input``) is shared with the
    other baselines; PathRAG keeps only its own index storage under
    ``project_dir/pathrag_storage``.  Like the KAG client, ``prepare``
    creates the otherwise-empty project directory so a fresh ``run`` follows
    the same prepare -> index workflow without tripping the preflight
    existence check.
    """
    (project_dir or DEFAULT_PROJECT_DIR).resolve().mkdir(parents=True, exist_ok=True)
...
def preflight(
    *,
    ...
    storage_dir = project_dir / "pathrag_storage"
    storage_markers = (
        "vdb_chunks.json",
    ...
    if missing_index:
        issues.append(f"PathRAG index is missing under {storage_dir}")
    if require_index and not missing_index:
        ...
        if not vector_files:
            issues.append(
                f"PathRAG index has no persisted vectors under {storage_dir}"
            )
    ...
    *,
    ...
    try:
        ...
        if result.has_errors:
            raise RuntimeError("PathRAG indexing failed:\n" + "\n".join(result.errors))
        return {
...
class SourceResolver:
    """Resolve PathRAG retrieved chunks to benchmark source IDs.

    PathRAG reports retrieved context as raw chunk text without file
    provenance, so resolution relies on the unified corpus: an embedded
    ``SOURCE_ID:`` header when the chunk kept it, otherwise containment in
    the corpus document whose text includes the chunk.
    """
    ...
    def __init__(
        ...
        # Accept the shared resolver's (documents, text_units, manifest)
        ...
        input_dir = corpus_input_dir(corpus_dir) if corpus_dir else None
        if input_dir and input_dir.is_dir():
            for path in sorted(input_dir.glob("*.txt")):
                text = path.read_text(encoding="utf-8")
                source = source_id_from_text(text) or path.stem.split("--", 1)[0]
                self._documents.append((safe_slug(source), text))

    def resolve(self, file_path: Any, text: str = "") -> str:
        source = source_id_from_text(text or "")
        if source:
            return source
        needle = (text or "").strip()
        if needle:
            for source_id, document in self._documents:
        ...
        return f"unresolved:{path.name or 'chunk'}"
...
def evaluate(
    *,
    ...
    resolver = SourceResolver(manifest, corpus_dir=corpus_dir or DEFAULT_CORPUS_DIR)
    client = PathRAGClient(project_dir, llm_overrides=llm_overrides, verbose=verbose)
    ...
    result = {
        "metadata": {
            "created_at": now_iso(),
            "pathrag_version": _pathrag_version(),
            "project_dir": str(project_dir),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build and evaluate the PathRAG prenatal benchmark"
    )
